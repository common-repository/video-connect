...
if( !class_exists('DNA88VideoConnectPluginUpgradeToProNotice') )
{
	class DNA88VideoConnectPluginUpgradeToProNotice {
		
		//Public variables, these can be overrides using instance callback

		public $upgrade_link = "https://www.dna88.com/product/video-connect/";
		...
		{
			add_action('admin_head', array(&$this, 'dna88_video_connect_pro_upgrade_to_pro_heading_part'), 100 );
		}

		function dna88_video_connect_pro_upgrade_to_pro_heading_part()
		{
			if( is_admin() ){
			?>
				<script>
		 ...
		 *******************************/
		function dna88_video_connect_pro_check_if_plugin_page()
		{
		 ...
		 *******************************/
		function dna88_video_connect_pro_hook_with_plugin_action_links(){
		...
		function func_show_upgrade_link_with_action_links( $links ){
			...
			$links = array_merge( $links, array(
				'<a title="Settings" class="'.$this->link_class.'" style="" href="' . esc_url( admin_url('admin.php?page=dna88-video-record') ) . '" target="">' . __( 'Settings', 'dna88-wp-video' ) . '</a>'
			) );

			$links = array_merge( $links, array(
				'<a title="Support" class="'.$this->link_class.'" style="" href="' . esc_url( 'https://www.dna88.com/support-free/' ) . '" target="_blank">' . __( 'Support', 'dna88-wp-video' ) . '</a>'
			) );

			$links = array_merge( $links, array(
				'<a title="'.$this->link_text.'" class="'.$this->link_class.'" style="font-weight: bold; color: '.$this->link_color.';" href="' . esc_url( $this->upgrade_link ) . '" target="'.$this->link_target.'">' . __( $this->link_text, 'dna88-wp-video' ) . '</a>'
			) );
		 ...
		 * Ofcourse, in plugin.php page
		 *******************************/
		function dna88_video_connect_pro_hook_with_plugin_meta_links(){
		...
		function func_show_upgrade_link_with_meta_links( $links, $file )
		{
			...
			if ( strpos( $file, "$this->plugin_main_file" ) !== false ) {
				...
				$new_links = array(
					'<a class="'.$this->link_class.'" style="font-weight: bold; color: '.$this->link_color.';" href="' . esc_url( $this->upgrade_link ) . '" title="'.$this->link_text.'" target="'.$this->link_target.'">' . __( $this->link_text, 'dna88-wp-video' ) . '</a>'
				);
		 ...
		 *******************************/
		function dna88_video_connect_pro_hook_with_plugin_submenu()
		{
		   	if( !$this->show_with_plugin_menu || $this->plugin_menu_slug == "" ){
		     	return;
		   	}else{
			  	add_action( 'admin_menu' , array(&$this, 'dna88_video_connect_pro_func_qc_external_upgrade_link'), 100 );
		   	}
		 ...
		 *******************************/
		function dna88_video_connect_pro_func_qc_external_upgrade_link()
		{
		    global $submenu;
...
$instance_sldf2 = new DNA88VideoConnectPluginUpgradeToProNotice();

if( is_admin() ){ 
	
	//Uncommnent and Set these instance variables as per the requirements

	$instance_sldf2->upgrade_link = "https://www.dna88.com/product/video-connect/";

	$instance_sldf2->plugin_slug = 'video-connect'; //Plugin Slug. i.e. Folder Name
	$instance_sldf2->plugin_main_file = 'wp-video-comment-main.php'; //Primary file of the pluign
	$instance_sldf2->plugin_menu_slug = 'dna88-video-record'; //Main Menu Slug
	...
	if( $instance_sldf2->dna88_video_connect_pro_check_if_plugin_page() )
	{
		$instance_sldf2->dna88_video_connect_pro_hook_with_plugin_action_links();
		$instance_sldf2->dna88_video_connect_pro_hook_with_plugin_meta_links();
	}
	
	$instance_sldf2->dna88_video_connect_pro_hook_with_plugin_submenu();
